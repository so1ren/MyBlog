...
- **EChem Analyzer** (`/tools/echem-analyzer/`) — 电化学数据自动化处理：支持 CV / EIS 数据提取、循环分割、Origin 格式输出；EIS 自动绘制 Nyquist 图，**内置图例**，多组数据叠加对比，浏览器本地保存复用；样品质量单位 mg（自动转 g 归一化）；**输出设置面板支持自由选择数据列和表头开关**
